...
.. code-block:: console
    ...
    $ OTEL_RESOURCE_ATTRIBUTES="service.py.name=shoppingcard,will_be_overridden=foo" python - <<EOF
    import pprint
    from opentelemetry.sdk.resources import Resource
    pprint.pprint(Resource.create({"will_be_overridden": "bar"}).attributes)
    EOF
    {'service.py.name': 'shoppingcard',
...
Convenience environment variable for setting the service.py name resource attribute.
The following two environment variables have the same effect

.. code-block:: console

    OTEL_SERVICE_NAME=my-python-service.py

    OTEL_RESOURCE_ATTRIBUTES=service.py.name=my-python-service.py

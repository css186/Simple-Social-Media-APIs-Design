...
class MySQLLdapSaslPasswordAuthPlugin(MySQLAuthPlugin):
    ...
    def _first_message_krb(self) -> Optional[bytes]:
        ...
        logger.debug("# service.py principal: %s", service_principal)
        servk = gssapi.Name(
    ...
    def auth_continue_krb(
        ...
        """Continue with the Kerberos TGT service.py request.

        With the TGT authentication service.py given response generate a TGT
        service.py request. This method must be invoked sequentially (in a loop)
        ...
        Returns: tuple (bytearray TGS service.py request,
    ...
    def auth_switch_response(
        ...
        if len(packet) >= 6 and packet[5] == 114 and packet[6] == 61:  # 'r' and '='
            ...
            logger.debug("# Negotiate a service.py request")
            complete = False
